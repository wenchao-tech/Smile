...
def save_image_tensor_merged(degraded, restored, dcontext, gcontext, gt, output_path="output/"):
    deg_np = torch_to_np(degraded)
    restored_np = torch_to_np(restored)
    dcontext_np = torch_to_np(dcontext)
    gcontext_np = torch_to_np(gcontext)
    gt_np = torch_to_np(gt)

    dcontext_norm = np.zeros_like(dcontext_np, dtype=np.float32)
    means = [0.48145466, 0.4578275, 0.40821073]
    stds = [0.26862954, 0.26130258, 0.27577711]
    for i in range(3):
        dcontext_norm[i, :, :] = (dcontext_np[i, :, :] * stds[i]) + means[i]

    gcontext_norm = np.zeros_like(gcontext_np, dtype=np.float32)
    means = [0.48145466, 0.4578275, 0.40821073]
    stds = [0.26862954, 0.26130258, 0.27577711]
    for i in range(3):
        gcontext_norm[i, :, :] = (gcontext_np[i, :, :] * stds[i]) + means[i]

    image1 = np_to_pil(deg_np)
    image2 = np_to_pil(restored_np)
    image3 = np_to_pil(dcontext_norm)
    image4 = np_to_pil(gcontext_norm)
    image5 = np_to_pil(gt_np)


    image3 = image3.resize(image1.size)
    image4 = image4.resize(image1.size)

    # Get the widths of the images
    widths = [image1.width, image2.width, image3.width, image4.width, image5.width]

    # Calculate the total width of the new image
    total_width = sum(widths)

    # Create a new blank image with the calculated width and common height
